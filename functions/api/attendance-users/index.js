...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const result = await db.prepare("SELECT id, login, name, role, course, module, active, created_at AS createdAt, updated_at AS updatedAt FROM attendance_users ORDER BY name").all();
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const course = String(body.course || "").toUpperCase();
    const module = course === "CFO" ? String(body.module || "") : "";
    if (!login || !name || password.length < 6) return json({ error: "Informe nome, login e uma senha com pelo menos 6 caracteres." }, { status: 400 });
    if (!allowedCourses.includes(course)) return json({ error: "Curso invalido." }, { status: 400 });
    if (course === "CFO" && !cfoModules.includes(module)) return json({ error: "Selecione o modulo do CFO." }, { status: 400 });
    const id = body.id || crypto.randomUUID();
    const passwordHash = await hashPassword(password);
    await db.prepare(`INSERT INTO attendance_users (id, login, name, password_hash, role, course, module, active)
      VALUES (?, ?, ?, ?, 'professor', ?, ?, 1)
      ON CONFLICT(login) DO UPDATE SET name=excluded.name, password_hash=excluded.password_hash,
      course=excluded.course, module=excluded.module, active=1, updated_at=datetime('now')`)
      .bind(id, login, name, passwordHash, course, module).run();
    return json({ ok: true });
  } catch (error) {
